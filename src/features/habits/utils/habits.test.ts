...
describe("habits utils", () => {
  ...
  it("checks habit logs by habit and date", () => {
    const logs: LogEntry[] = [
      { habitId: 1, date: "2026-03-06" },
      { habitId: 2, date: "2026-03-05" },
    ];

    expect(isLogged(logs, 1, "2026-03-06")).toBe(true);
    expect(isLogged(logs, 1, "2026-03-05")).toBe(false);
  ...
  it("computes streak for consecutive days ending today", () => {
    const logs: LogEntry[] = [
      { habitId: 1, date: "2026-03-06" },
      { habitId: 1, date: "2026-03-05" },
      { habitId: 1, date: "2026-03-04" },
      { habitId: 2, date: "2026-03-06" },
    ];

    expect(getStreak(logs, 1)).toBe(3);
  });
  ...
  it("returns zero streak when today is not logged", () => {
    ...
    expect(getStreak(logs, 1)).toBe(0);
  });
